  ...
  return (
    <tr className="border-b-2 border-gray-200 font-semibold last:border-0">
      {unRankedTeam ? (
        <>
          <td
            className={cn("-pr-2 hidden w-52 pl-2 sm:block", {
              [MLBstyleData[unRankedTeam].primaryBGstyle]: true,
              [MLBstyleData[unRankedTeam].primaryPlainText]: true,
            })}
          >
            {MLBteamData[unRankedTeam].location}{" "}
            {MLBteamData[unRankedTeam].name}
          </td>
          <td
        ...
        <td className={cn("w-24 bg-mlb/30 sm:w-52")}></td>
      )}
      <td>
        <div className="flex justify-center overflow-hidden rounded bg-mlb">
          <input
            type="number"
            min={1}
            max={32}
      ...
      <td>
        <div className="flex justify-between">
          <button
            className="rounded px-1 py-0.5 text-mlb"
            onClick={() => {
              mlbRankDispatch({
                type: "UNRANK_TEAM",
                payload: {
      ...
      {rankedTeam ? (
        <>
          <td
            className={cn("-pr-2 hidden w-52 pl-2 sm:block", {
              [MLBstyleData[rankedTeam].primaryBGstyle]: true,
              [MLBstyleData[rankedTeam].primaryPlainText]: true,
            })}
          >
            {MLBteamData[rankedTeam].location} {MLBteamData[rankedTeam].name}
          </td>
          <td
            className={cn("-pr-2 w-24 pl-2 sm:hidden", {
              [MLBstyleData[rankedTeam].primaryBGstyle]: true,
