import { driverCodes2025, constructorNames2025, calculatePoints } from "~/data/F1/2025/F1data";
import type { DriverCode25Type, ConstructorName25Type } from "~/data/F1/2025/F1data";
import { type RaceResultsType, FormulaOneRaceResults } from "~/data/F1/2025/raceData";
import { cumulativeArray } from "~/utils/cuA";
import { dcSort } from "~/utils/sorting";

type FullDriverResultType = {
  driverName: DriverCode25Type | undefined;
  constructor: ConstructorName25Type | undefined;
...
export const resultsSortedByDriver = (results: RaceResultsType, order: DriverCode25Type[]) => {
  const findPlaceArray = (driver: DriverCode25Type) => {
...
export const createStandings = (results: RaceResultsType) => {
  const driverStandings: {[Key in DriverCode25Type] : {total: number, runTotal: number[]}} = {
  ...
  const driverPlaces: {[Key in DriverCode25Type]?: number[]} = {};
  const constructorPlaces: {[Key in ConstructorName25Type]? : number[]} = {};

  const driverPodiums: {[Key in DriverCode25Type]?: number} = {};
  const constructorPodiums: {[Key in ConstructorName25Type]? : number} = {};

  const constructorStandings: {[Key in ConstructorName25Type] : {total: number, runTotal: number[]}} = {
  ...
  const constructorStandingsArray = constructorNames2025.map((constructor) => {
  ...
  const driverStandingsArray = driverCodes2025.map((driver) => {
    const driverInfo = {
      name: driver,
  ...
  // const runningConstructorResults = constructorOrder.map((constructor)=>{
  //   const driverPoints = constructorToDrivers2025(constructor).map((driver) => {
