import { driverCodes2026, constructorNames2026, calculatePoints } from "~/data/F1/2026/F1data";
import type { DriverCode26Type, ConstructorName26Type } from "~/data/F1/2026/F1data";
import { type RaceResultsType, FormulaOneRaceResults } from "~/data/F1/2026/raceData";
import { cumulativeArray } from "~/utils/cuA";
import { dcSort } from "~/utils/sorting";

type FullDriverResultType = {
  driverName: DriverCode26Type | undefined;
  constructor: ConstructorName26Type | undefined;
...
export const resultsSortedByDriver = (results: RaceResultsType, order: DriverCode26Type[]) => {
  const findPlaceArray = (driver: DriverCode26Type) => {
...
export const createStandings = (results: RaceResultsType) => {
  ...
  const driverPlaces: {[Key in DriverCode26Type]?: number[]} = {};
  const constructorPlaces: {[Key in ConstructorName26Type]? : number[]} = {};

  const driverPodiums: {[Key in DriverCode26Type]?: number} = {};
  const constructorPodiums: {[Key in ConstructorName26Type]? : number} = {};

  const constructorStandings: {[Key in ConstructorName26Type] : {total: number, runTotal: number[]}} = {
    Alpine : {total: 0, runTotal: [0]},
    "Aston Martin" : {total: 0, runTotal: [0]},
    Audi : {total: 0, runTotal: [0]},
    Cadillac : {total: 0, runTotal: [0]},
  ...
  results.forEach((result) => {
    ...
    finalOrder.forEach((dac, index) => {
      ...
      const addition = calculatePoints(index, sprint);
  ...
  const constructorStandingsArray = constructorNames2026.map((constructor) => {
  ...
  const driverStandingsArray = driverCodes2026.map((driver) => {
    const driverInfo = {
      name: driver,
  ...
  const findPlaceArray = (driver: DriverCode26Type) => {
    const driverArray: FullDriverResultType[] = results.map((result) => {
      ...
      const constructor = finalOrder[place]?.constructor;
      const points = calculatePoints(place, sprint);
      return {
        driverName: driver,
        constructor,
        sprint: sprint,
    ...
    const teamPoints = FormulaOneRaceResults.map((race) => {
      const teamResult = race.finalOrder.map((dac, index) => {
        return {
          ...
          points: calculatePoints(index, race.sprint)
        }
  ...
  // const runningConstructorResults = constructorOrder.map((constructor)=>{
  //   const driverPoints = constructorToDrivers2026(constructor).map((driver) => {
